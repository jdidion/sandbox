 ...
 * 
 * @see http://software.jessies.org/salma-hayek/
 */
public class ETable extends JTable {
     ...
     */
    public ETable(AbstractETableModel model) {
        super(model);
        
        // Although it's the JTable default, most systems' tables don't draw a grid by default.
        // Worse, it's not easy (or possible?) for us to take over grid painting ourselves for those LAFs (Metal, 
        // for example) that do paint grids. The Aqua and GTK LAFs ignore the grid settings anyway, so this causes no 
        // change there.
        setShowGrid(false);
        ...
        // Enable Java 6's row sorting by default
        // FIXME: this isn't safe unless callers are updated to take into account the possible view/model row index 
        // mismatches. (The selection indexes are in terms of the view, not the model.)
    ...
    private Component prepareComponent(Component c, int row, int column) {
        ...
        if (selected) {
            if (SwingUtils.isMacOs() && focused == false && isEditing() == false) {
            ...
            c.setBackground(SwingUtils.backgroundColorForRow(row));
            c.setForeground(UIManager.getColor("Table.foreground"));
        }
        
        if (!((AbstractETableModel) getModel()).isEnabled(row)) {
        ...
        if (c instanceof JComponent) {
            ...
            if (SwingUtils.isGtk() && c instanceof JCheckBox) {
                ...
                jc.setOpaque(true);
            } else if (SwingUtils.isMacOs() && c instanceof JCheckBox) {
